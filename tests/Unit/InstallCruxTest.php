...
namespace Etlok\Crux\Tests\Unit;

use Illuminate\Support\Facades\Artisan;
use Illuminate\Support\Facades\File;
use Etlok\Crux\Tests\TestCase;
...
class InstallCruxTest extends TestCase
{
    ...
    function the_install_command_copies_the_configuration()
    {
        ...
        if (File::exists(config_path('crux.php'))) {
            unlink(config_path('crux.php'));
        }

        $this->assertFalse(File::exists(config_path('crux.php')));

        Artisan::call('crux:install');

        $this->assertTrue(File::exists(config_path('crux.php')));
    }
    ...
    public function when_a_config_file_is_present_users_can_choose_to_not_overwrite_it()
    {
        ...
        File::put(config_path('crux.php'), 'test contents');
        $this->assertTrue(File::exists(config_path('crux.php')));

        // When we run the install command
        $command = $this->artisan('crux:install');
        ...
        $this->assertEquals('test contents', file_get_contents(config_path('crux.php')));
    ...
    public function when_a_config_file_is_present_users_can_choose_to_do_overwrite_it()
    {
        ...
        $this->assertEquals(
            ...
            file_get_contents(config_path('crux.php'))
        );
